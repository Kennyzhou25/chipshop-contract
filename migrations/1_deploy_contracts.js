...
const migrationStartBlock = 11081229;
const migrationEndBlock = 11137000;
const epochStartTime = 1632333600;
const chipAllocationAmount = 300;
const fishAllocationAmount = 164;
...
async function beforeMigration(deployer, network) {
  ...
  await deployer.deploy(TokenMigration, oldChipAddress, oldFishAddress, oldMpeaAddress, Chip.address, Fish.address, migrationStartBlock, migrationEndBlock);
...
async function afterMigration(deployer, network) {
  ...
  await deployer.deploy(FishRewardPool, fishAddress, migrationEndBlock);
